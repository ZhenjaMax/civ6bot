...
export class NewCommands{
    ...
    async newFFA(interaction: CommandInteraction){
        await this.newService.getNewFFA(interaction);
    }

    @Slash("teamers", { description: "Начало игры Teamers" })
    async newTeamers(interaction: CommandInteraction){
        await this.newService.getNewTeamers(interaction);
    }
}
